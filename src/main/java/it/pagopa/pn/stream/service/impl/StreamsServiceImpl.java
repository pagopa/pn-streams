...
import it.pagopa.pn.stream.middleware.dao.webhook.dynamo.mapper.EntityToDtoStreamMapper;
import it.pagopa.pn.stream.middleware.dao.webhook.dynamo.mapper.EntityToStreamListDtoStreamMapper;
...
@Service
@Slf4j
public class StreamsServiceImpl extends StreamServiceImpl implements StreamsService {
    ...
    private void postConstruct() {
        PnStreamConfigs.Webhook webhookConf = pnStreamConfigs.getWebhook();
            ...
            .flatMap(filteredEntity ->
                 streamEntityDao.delete(xPagopaPnCxId, streamId.toString())
                    .then(Mono.fromSupplier(() -> {
                        schedulerService.scheduleWebhookEvent(streamId.toString(), null, purgeDeletionWaittime, StreamEventType.PURGE_STREAM);
                        return null;
            })))
    ...
    public Mono<StreamMetadataResponseV25> getEventStream(String xPagopaPnUid, String xPagopaPnCxId, List<String> xPagopaPnCxGroups, String xPagopaPnApiVersion, UUID streamId) {
        ...
        return streamEntityDao.get(xPagopaPnCxId, streamId.toString())
            .switchIfEmpty(Mono.error(new PnStreamStreamNotFoundException("Stream  "+streamId.toString() +" not found ")))
            .filter(streamEntity -> entityVersion (streamEntity).equals(apiVersion(xPagopaPnApiVersion)))
            .switchIfEmpty(Mono.error(new PnStreamForbiddenException("Stream  "+streamId.toString() +" cannot be accessed by  xPagopaPnCxId="+xPagopaPnCxId)))
        ...
        return streamEntityDao.findByPa(xPagopaPnCxId)
            .filter(streamEntity -> streamEntity.getDisabledDate() == null)
                .collectList().flatMap(list -> {
                    if (list.size() >= pnStreamConfigs.getWebhook().getMaxStreams()) {
    ...
    private Mono<StreamEntity> replaceStreamEntity(StreamEntity entity, StreamEntity replacedStream) {
        if (replacedStream.getDisabledDate() != null){
            ...
            entity.setEventAtomicCounter(replacedStream.getEventAtomicCounter() + pnStreamConfigs.getWebhook().getDeltaCounter());
